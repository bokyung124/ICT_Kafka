...
class ConfigManager:
    ...
    def load_config(self, config_path):
        ...
        try:
            ...
            self._logger.set_initialize(
                config_logger.path,
                config_logger.level,
                config_logger.max_size,
                config_logger.file_count,
                config_logger.console_log,
                config_logger.data_dump,
            )
            self.version()
            self._logger.info(f"[{myfunc}] Config file: ({config_path})")
            self._logger.info(
                f"[{myfunc}] Log path: {config_logger.path}, level: {config_logger.level}, max size: {config_logger.max_size}, file count: {config_logger.file_count}, console log: {config_logger.console_log}"
            )

            # db
            self._db_connection_info = self.get_db_connection_info()
            self._logger.debug(
                f"[{myfunc}] DB Connection Config {self._db_connection_info}"
            )

            # kafka
            self._kafka_connection_info = self.get_kafka_connection_info()
            self._logger.debug(
                f"[{myfunc}] Kafka Connection Config {self._kafka_connection_info}"
            )

            self._commit_timeout = self.get_kafka_commit_timeout()
            self._transaction_timeout = self.get_kafka_transaction_timeout()
            self._logger.debug(
                f"[{myfunc}] Kafka Config. commit timeout: {self._commit_timeout} sec, transaction timeout: {self._transaction_timeout} sec"
            )

            # monitor
            self._monitor_url = self.get_monitor_url()
            self._logger.debug(f"[{myfunc}] Monitor URL: {self._monitor_url}")

            # interface
            self.interface_list = self.get_interface_info()
            for lst in self.interface_list:
                self._logger.debug(
                    f"[{myfunc}] Interface Config. interface id:{lst.intf_id}"
                )
    ...
    def get_kafka_connection_info(self):
        ...
        for element in self._rootDocument.findall(root_find_path):
            ...
            transaction_timeout_ms = self.get_attrubute(
                element, "transaction.timeout.ms"
            )
            ...
            connections_max_idle_ms = self.get_attrubute(
                element, "connections.max.idle_ms"
            )
            ...
            queue_buffering_max_messages = self.get_attrubute(
                element, "queue.buffering.max.messages"
            )
            queue_buffering_max_kbytes = self.get_attrubute(
                element, "queue.buffering.max.kbytes"
            )
            queue_buffering_max_ms = self.get_attrubute(
                element, "queue.buffering.max.ms"
            )
            message_send_max_retries = self.get_attrubute(
                element, "message.send.max.retries"
            )
            ...
            if connections_max_idle_ms != 0 and connections_max_idle_ms != "":
                self._kafka_dict["connections.max.idle.ms"] = str(
            ...
            if queue_buffering_max_messages != 0 and queue_buffering_max_messages != "":
                self._kafka_dict["queue.buffering.max.messages"] = str(
                    queue_buffering_max_messages
                )
            if queue_buffering_max_kbytes != 0 and queue_buffering_max_kbytes != "":
                self._kafka_dict["queue.buffering.max.kbytes"] = str(
            ...
            if message_send_max_retries != 0 and message_send_max_retries != "":
                self._kafka_dict["message.send.max.retries"] = str(
    ...
    def get_kafka_commit_timeout(self):
        ...
        for element in self._rootDocument.findall(root_find_path):
            commit_timeout = int(
                int(self.get_attrubute(element, "internal.commit.timeout")) / 1000
            )
    ...
    def get_kafka_transaction_timeout(self):
        ...
        for element in self._rootDocument.findall(root_find_path):
            transaction_timeout = int(
                int(self.get_attrubute(element, "internal.transaction.timeout")) / 1000
            )
    ...
    def get_monitor_url(self):
        """
        read monitor server URL configuration
        """
        root_find_path = "./connection/monitor"
        elements = self._rootDocument.findall(root_find_path)
        if len(elements) > 0:
            url = self.get_attrubute(elements[0], "url")
            if url != 0 and url != "":
                return str(url)
        return ""
    ...
    def get_interface_info(self):
        ...
        for interface in self._rootDocument.findall(root_find_path):
            ...
            config_interface.process_type = self.get_attrubute(
