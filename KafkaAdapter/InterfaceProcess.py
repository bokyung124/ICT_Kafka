...
class InterfaceProcess:
    def __init__(self):
        ...
        self._monitor_url = self._config_manager._monitor_url
        ...
        try:
            ...
            for lst in self._interface_list:
                if lst.intf_type == "DBGET":
                    self._logger.info(
                        f"[{myfunc}] DBGET Interface start. interface id:{lst.intf_id}"
                    )
                    self._kafka_wrapper.kafka_connect(
                        self._config_manager.get_kafka_connection_info()
                    )
                    self._kafka_wrapper.kafka_init_transaction()
                    self._mysql_wrapper.db_connect(
                        self._config_manager.get_db_connection_info()
                    )
                    ...
                    while True:
                        ...
                        if len(result) > 0:
                            ...
                            self._logger.info(
                                f"[{myfunc}] DBGET select success. interface id:{lst.intf_id}. data:[{result}]"
                            )

                            self._kafka_wrapper.kafka_put(
                                lst.intf_out, json.dumps(result)
                            )
                            self._logger.info(
                                f"[{myfunc}] DBGET kafka put success. interface id:{lst.intf_id}"
                            )
                            ...
                            self._logger.debug(
                                f"[{myfunc}] DBGET Empty. interface id:{lst.intf_id}."
                            )
                            self._mysql_wrapper.db_commit()
                        ...
                        if self._monitor_url:
                            try:
                                monitor_status = (
                                    "SUCCESS" if len(result) > 0 else "EMPTY"
                                )
                                monitor_body = {
                                    "id": str(uuid.uuid4()),
                                    "intf_id": lst.intf_id,
                                    "intf_name": lst.intf_id,
                                    "host_id": socket.gethostname(),
                                    "process_dt": self._common_util.current_datetime(),
                                    "status": monitor_status,
                                    "error_message": "",
                                }
                                self._rest_client.restapi_post_normal(
                                    self._monitor_url, monitor_body
                                )
                                self._logger.debug(
                                    f"[{myfunc}] Monitor info sent. status:{monitor_status}"
                                )
                            except Exception as monitor_err:
                                self._logger.warning(
                                    f"[{myfunc}] Monitor info send failed. err: ({monitor_err})"
                                )
                        if lst.process_type == "realtime":
                            pass
                        else:
            ...
            self._logger.error(
                f"[{myfunc}] data_get error. err: ({err}), interface id:{lst.intf_id})"
            )
            raise
